...
# from pt20200719.utils import loadWord2Vec, clean_str
...
####  这里是建立每个词出现在样本的关系，比如词语“科技”出现在样本22喝23中，则
###   word_doc_list中记录【‘科技’：【22，23，29，30000，300000，29999，10000】】
...
for i in range(real_train_size):
    ...
    doc_words = shuffle_doc_words_list[i] #shuffle_doc_words_list按照 ID存储了所有的内容
    words = doc_words.split()
    doc_len = len(words)
    for word in words:
        ...
        doc_vec = doc_vec + np.array(word_vector)#这里求得是文本节点得向量，文本节点得向量是将文本中所有词向量相加
...
for i in range(train_size):#（real train + validation dataset）
    doc_vec = np.array([0.0 for k in range(word_embeddings_dim)])
    doc_words = shuffle_doc_words_list[i]
    words = doc_words.split()
    doc_len = len(words)
    ...
    for j in range(word_embeddings_dim):
        ...
        #这里反正都是0了，是否可以不用加？ allx就只是存储词语的向量

#feature 是代表了整个图中的训练的节点
for i in range(vocab_size):
    for j in range(word_embeddings_dim):
