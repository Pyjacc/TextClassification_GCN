...
'''
首先对每一行求和得到rowsum；求倒数得到r_inv；如果某一行全为0，则r_inv算出来会等于无穷大，
将这些行的r_inv置为0；构建对角元素为r_inv的对角矩阵；用对角矩阵与原始矩阵的点积起到标准
化的作用，原始矩阵中每一行元素都会与对应的r_inv相乘。
'''
...
def preprocess_adj(adj):
    ...
    #这里考虑了自联结，也就是加一个单位矩阵，可以看到结果，矩阵的对角线元素均增大了1
